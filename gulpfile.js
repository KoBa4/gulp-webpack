...
// Config
const autoprefixConfig = { overrideBrowserslist: supportedBrowsers, cascade: false };
...
const srcPath = (file, watch = false) => {
  ...
  if (file === 'fonts') return './src/fonts/**/*.ttf';
...
const distPath = (file, serve = false) => {
  ...
  if (file === 'fonts') return './dist/fonts';
...
const cleanFonts = (mode) => () => {
  return ['development', 'production'].includes(mode) ? del([distPath('fonts')]) : undefined;
};
...
const buildImages = (mode) => (done) => {
  ['development', 'production'].includes(mode) ? pump([
    ...
    gulpImagemin([
      ...
      gulpImagemin.svgo({ removeViewBox: false }),
      imageminPngquant(),
...
const buildStyles = (mode) => (done) => {
  ...
  ['development', 'production'].includes(mode) ? pump([
    ...
    ...((mode === 'development') ? [gulpSourcemaps.init({ loadMaps: true })] : []),
    gulpSass.sync({ outputStyle })
      .on('error', notify.onError()),
    gulpPostcss(postcssPlugins),
    ...((mode === 'development') ? [gulpSourcemaps.write('./')] : []),
    // ...((mode === 'production') ? [] : []),
    // cleanCSS({ level: 2 }),
